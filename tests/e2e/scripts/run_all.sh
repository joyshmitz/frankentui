...
# Check the selected suites before a missing tool can look like an assertion failure.
required_tools=(bash cargo "${E2E_PYTHON:-python3}" jq rg awk sed grep wc tr cat xxd sha256sum)
if $VERBOSE; then
    required_tools+=(tee)
fi
if ! $QUICK; then
